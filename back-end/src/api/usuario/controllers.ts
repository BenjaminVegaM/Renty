...
async function isEmailRegistered(req: Request, res: Response): Promise<void>
{
  ...
  const retData = await StartQuery("SELECT id FROM usuario WHERE email = '" + email + "';");
...
async function getUserFromToken(req: Request, res: Response): Promise<void>
{
  ...
  const retData = await StartQuery("SELECT usuario.id, usuario.nombre, usuario.email FROM usuario JOIN tokens ON usuario.id = tokens.id_cuenta WHERE token = '" + token + "';");
  const user = retData[0];
...
async function createAccount(req: Request, res: Response): Promise<void>
{
  ...
  // Tries to get a user with the email
  var retData = await StartQuery("SELECT id FROM usuario WHERE email = '" + email + "';");
  var user = retData[0];
  console.log("User =", user);
  ...
  retData = await StartQuery(`INSERT INTO usuario (nombre, email, contrasenya) VALUES ('${name}', '${email}', '${hashedPass}');`);
  
  console.log("User inserted successfully");

  const idData = await StartQuery("SELECT id FROM usuario WHERE email = '" + email + "';");
...
async function logIn(req: Request, res: Response): Promise<void>
{
  ...
  // Tries to get a user with the email
  const retData = await StartQuery("SELECT id, nombre, email, contrasenya FROM usuario WHERE email = '" + email + "';");
  const user = retData[0];
  console.log("User =", user);
  ...
  //console.log("User.Password  =", user.contrasenya);
...
async function modifyAccount(req: Request, res: Response): Promise<void>
{
  ...
  // Checks if the email is already registered
  // Tries to get a user with the email
  var emailData = await StartQuery(`SELECT id FROM usuario WHERE email = '${email}' AND id <> ( SELECT id_cuenta FROM tokens WHERE token = '${token}' );`);
  ...
  // Tries to get the user using the token
  const updData = await StartQuery(`UPDATE usuario SET nombre = '${name}', email = '${email}', contrasenya = '${hashedPass}' WHERE id = ( SELECT id_cuenta FROM tokens WHERE token = '${token}' );`);
...
async function deleteAccount(req: Request, res: Response): Promise<void>
{
  ...
  // Tries to get the user using the token
  const retData = await StartQuery("DELETE FROM usuario WHERE id IN ( SELECT id_cuenta FROM tokens WHERE token = '" + token + "' );");
  const user = retData[0];
